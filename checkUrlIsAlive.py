...
def send(url_line):
    ...
    try:
        ...
        # 避免陷入死循环
        if request_count.get(url[:20]) is None:
            request_count[url[:20]] = 0
        else:
            request_count[url[:20]] = request_count[url[:20]] + 1
        print(url + '  已请求   ' + str(request_count[url[:20]]))
        if request_count[url[:20]] > 3:
            raise Exception
